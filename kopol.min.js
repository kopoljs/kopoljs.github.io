...
function readData(message, callback){
   ...
   $.get(
        'https://bahadorsaket.com/others/ranking2.txt',
        function (response) {
            allText =response;
            ...
            for (var i=1; i<allTextLines.length; i++) {
              ...
              if (data.length == headers.length) {
                  ...
                  metrics.push([data[4], data[5], data[6]])
              }
            }
            var effectivness = calculateEffectiveness(metrics, message[3],message[4],message[5]);
...
function getChartPeformance(message,callback){
  ...
  readData(message, function(effectivness, features){
...
function calculateEffectiveness(metrics, alpha, beta, gama){
	var scores = [];
	for(i=0;i<metrics.length;i++)
	{
		var time = alpha * (1- (Number(metrics[i][2])/215));
		var accuracy = beta * (Number(metrics[i][0])/100);
		var perference = gama * (Number(metrics[i][1])/5);

		var score = (accuracy + perference + time)/3;
		if(score>=0.9)
		{
		 	scores.push("Very Good");
		}
...
function getRecommendations(message, features, effectivness, model,callback){
  ...
  var charts= ["PieChart","BarChart","LineChart","Scatterplot","Table"];
      ...
      for(j=0;j<charts.length;j++)
      {
        ...
        console.log(model.classify([message[0],message[1],message[2],charts[j]]),charts[j])
